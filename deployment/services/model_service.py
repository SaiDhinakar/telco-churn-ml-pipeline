...
def initialize_model_service():
    ...
    if server_running:
        print(f"🚀 MLflow server detected at {mlflow_host}:{mlflow_port}")
        ...
        try:
            ...
            print(f"✅ Fallback model loaded from {fallback_model_uri}")
    else:
        print("🏠 No MLflow server detected, using local artifact paths")
        ...
        try:
            ...
            print(f"✅ Champion model loaded from local path: {champion_model_path}")
        except Exception as e:
            print(f"❌ Failed to load champion model from local path: {e}")
            print("🔄 Using fallback model...")
            model = mlflow.pyfunc.load_model(fallback_model_path)
            print(f"✅ Fallback model loaded from local path: {fallback_model_path}")
